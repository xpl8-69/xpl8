...
function _initHardNotEgg(){
  document.addEventListener('click',function(e){
    const el=e.target;
    // بيـtrigger على diff-badge اللي فيها كلمة "hard" أو على hdr-title
    const isHardBadge = el && (
      el.classList.contains('diff-badge') ||
      el.classList.contains('d-hard') ||
      el.classList.contains('hdr-title') ||
      el.closest('.hdr-title')
    ) && (
      (el.classList.contains('diff-badge') || el.classList.contains('d-hard'))
