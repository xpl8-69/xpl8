...
const CHALLENGES = {
  ...
  6:{points:150,hints:[{cost:25,text:'look at the request being sent when you click verify. what parameters does the endpoint accept?'},{cost:30,text:'try adding an unexpected parameter to the request body — developers sometimes leave debug flags active.'}]},
  7:{points:175,hints:[{cost:30,text:'SQL injection is about breaking out of the string context. what character ends a string in SQL?'},{cost:35,text:'try: admin\'-- in the username field. the double dash comments out the rest of the query.'}]},
  8:{points:175,hints:[{cost:30,text:'the endpoint takes a resource ID. you can only see resource 1 and 3 as a guest.'},{cost:35,text:'try accessing resource 2 directly. authorization should be enforced server-side, but is it?'}]},
  ...
  10:{points:250,hints:[{cost:50,text:'the proof is a SHA-256 hash. look at the clue endpoint — it tells you what to hash.'},{cost:60,text:'proof = SHA256("0x69er" + first 8 chars of your session token). your token is in the x-session header.'}]},
};
const RESOURCES = {
...
app.post('/api/init', (req,res)=>{
  const {resetFull, deathCount, name} = req.body||{};
...
app.post('/api/check/:stage',(req,res)=>{
  ...
  if(n===11||n===12){
    ...
    // Case-insensitive hint: check if they got it right but with wrong case
...
<div class="top">
  <div>
    ...
    <div class="top-sub">0x69er CTF — restricted access</div>
  </div>
...
<div id="pane-visitors" class="pane">
  <div class="section-head">
    ...
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
      <input class="search" id="visitor-search" placeholder="search ip / name / country / gpu..." oninput="filterVisitors()">
...
function filterVisitors() {
  ...
  if (!q) return renderVisitors(_allVisitors);
  renderVisitors(_allVisitors.filter(v =>
...
async function loadPlayers() {
  ...
  const d = await r.json();
  if (!d.players.length) {
    document.getElementById('players-content').innerHTML = '<span style="color:var(--mu)">no active players.</span>';
    return;
  }
    ...
    <table>
      ...
      <tbody>
        \${d.players.map(p=>\`<tr>
          ...
          <td style="color:var(--blue)">\${esc(p.name)}</td>
          <td style="color:var(--yellow)">\${esc(p.ip)}</td>
          <td>\${dash(p.country)}</td>
          <td style="color:var(--mu)">\${dash(p.city)}</td>
          <td style="color:var(--mu)">\${dash(p.isp)}</td>
          <td>\${dash(p.os)}</td>
          <td>\${dash(p.browser)}</td>
          <td><span class="badge badge-purple">\${esc(p.device)||'?'}</span></td>
          <td style="color:var(--mu)">\${dash(p.screen)}</td>
          <td style="color:var(--mu)">\${dash(p.platform)}</td>
          ...
          <td style="color:var(--mu)">\${dash(p.timezone)}</td>
          <td style="color:var(--mu)">\${esc((p.lang||'').slice(0,10))}</td>
