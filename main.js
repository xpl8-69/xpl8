...
function _part2Screen(){
  _playRBD();
  document.getElementById('root').innerHTML='<div id="p2-screen" style="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;overflow:hidden;"><img id="subaru-img" src="'+_SUBARU_B64+'" style="width:min(320px,75vw);opacity:0;border-radius:4px;filter:grayscale(1) contrast(1.15) brightness(.85);transition:opacity 2.4s ease;margin-bottom:28px;"/><div id="p2-text" style="opacity:0;font-family:IBM Plex Mono,monospace;text-align:center;transition:opacity 2s ease 1.2s;"><div style="font-size:clamp(11px,2vw,14px);color:#333;letter-spacing:5px;margin-bottom:10px;">WAIT FOR</div><div style="font-size:clamp(20px,5vw,40px);color:#777;letter-spacing:10px;font-weight:700;">PART 2</div><div style="margin-top:18px;font-size:10px;color:#2a2a2a;letter-spacing:2px;line-height:2.2;">u wont have a chance next time.<br>not even close.</div></div><div id="p2-btn" style="opacity:0;margin-top:40px;transition:opacity 1.5s ease;"></div></div>';
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    const img=document.getElementById('subaru-img');if(img)img.style.opacity='1';
    setTimeout(()=>{const t=document.getElementById('p2-text');if(t)t.style.opacity='1';},800);
    setTimeout(()=>{
      _stopRBD();
      const img2=document.getElementById('subaru-img');if(img2){img2.style.transition='opacity 1.8s ease';img2.style.opacity='0';}
      const t=document.getElementById('p2-text');if(t){t.style.transition='opacity 1.8s ease';t.style.opacity='0';}
      setTimeout(()=>{
        const btn=document.getElementById('p2-btn');
        if(btn){btn.innerHTML='<button class="btn btn-o" onclick="_init(true)" style="font-size:14px;letter-spacing:3px;padding:14px 32px;">play again?</button>';btn.style.opacity='1';}
      },1900);
    },5800);
  }));
}
...
function _win(flag,score,maxScore){
  ...
  if(AC){try{[[261,0],[329,.1],[392,.2],[523,.35],[659,.5],[784,.65]].forEach(([f,d])=>{const o=AC.createOscillator(),g=AC.createGain();o.connect(g);g.connect(AC.destination);o.type='triangle';o.frequency.setValueAtTime(f,AC.currentTime+d);g.gain.setValueAtTime(0.15,AC.currentTime+d);g.gain.exponentialRampToValueAtTime(0.001,AC.currentTime+d+2.8);o.start(AC.currentTime+d);o.stop(AC.currentTime+d+3);});}catch(e){}}
  for(let i=0;i<10;i++)setTimeout(()=>{_particle(Math.random()*window.innerWidth,Math.random()*window.innerHeight*.7,'#6ee7a0',14);_particle(Math.random()*window.innerWidth,Math.random()*window.innerHeight*.7,'#fbbf24',9);_particle(Math.random()*window.innerWidth,Math.random()*window.innerHeight*.7,'#a78bfa',7);},i*160);
  document.getElementById('root').innerHTML=`
<div id="glow"></div>
<div class="app">
  <div class="win">
